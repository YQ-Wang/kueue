...
func TestCacheWorkloadOperations(t *testing.T) {
	...
	steps := []struct {
		...
		{
			name: "delete self-heals after clusterQueue deletion",
			operation: func(log logr.Logger, cache *Cache) error {
...
func TestLocalQueueUsage(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if wantExists := tc.cq != nil; gotUsage.ClusterQueueExists != wantExists {
				t.Errorf("ClusterQueueExists = %t, want %t", gotUsage.ClusterQueueExists, wantExists)
			}
			if tc.cq != nil && gotUsage.ClusterQueueUID != tc.cq.UID {
				t.Errorf("ClusterQueueUID = %q, want %q", gotUsage.ClusterQueueUID, tc.cq.UID)
			}
...
func TestDeleteClusterQueueClearsWorkloadBookkeeping(t *testing.T) {
	...
	if !cache.DeleteClusterQueue(cq) {
