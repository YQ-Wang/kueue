	...
	for _, tc := range cases {
		for _, unadmittedWorkloadsObservabilityEnabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/observability_%t", tc.name, unadmittedWorkloadsObservabilityEnabled), func(t *testing.T) {
				...
				tc.e.Head = wInfos[0]
				tc.e.clusterQueueIncarnationObserved = true
				tc.e.clusterQueueIncarnationEpoch = snapshot.ClusterQueueIncarnationEpoch
...
func TestStaleClusterQueueIncarnationBlocksSchedulerMutations(t *testing.T) {
	...
	for name, run := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if !cqCache.DeleteClusterQueue(oldCQ) {
...
func TestSiblingClusterQueueReplacementBlocksAdmissionFromStaleSnapshot(t *testing.T) {
	...
	if !cqCache.DeleteClusterQueue(oldSiblingCQ) {
...
func TestVictimClusterQueueReplacementBlocksPreemptionFromStaleSnapshot(t *testing.T) {
	...
	if !cqCache.DeleteClusterQueue(oldVictimCQ) {
